...
export const saveBooking = async (booking) => {
  try {
    let expertId = booking.expertId;
    
    // Safety check if expertId is not a string or missing
    if (!expertId || typeof expertId !== 'string') {
      console.error('Error: expertId is missing or invalid:', expertId);
      throw new Error('Invalid expertId');
    }

    // Resolve expert profile UUID to the expert user owner_id because 
    // the bookings.expert_id foreign key references the auth/profiles users table.
    // If expertId is less than 10 chars, it's a mock expert (which doesn't exist in Supabase).
    // Note that bookings table has a NOT-NULL constraint on expert_id, so we must raise an error.
    const isMockExpert = expertId.length < 10;
    if (isMockExpert) {
      throw new Error('Cannot book a mock expert in database environment.');
    }

    // Fetch the expert record to see if the passed expertId is the profile ID (experts.id)
    // and resolve it to their owner_id (user ID).
    const { data: expertData, error: resolveError } = await supabase
      .from('experts')
      .select('owner_id')
      .eq('id', expertId)
      .maybeSingle();
    ...
    if (resolveError) {
      console.error('Error resolving expert profile ID to owner_id:', resolveError);
    } else if (expertData && expertData.owner_id) {
      console.log(`Resolved expert profile ID ${expertId} to owner_id ${expertData.owner_id}`);
      expertId = expertData.owner_id;
    }
    ...
    console.error('saveBooking caught exception:', error);
    throw error; // Rethrow to let the UI react (close modal, show error toast)
  }
};
